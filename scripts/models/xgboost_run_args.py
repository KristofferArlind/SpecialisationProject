...
if (detached_test_dates):
    ...
    df_importances = pd.DataFrame(columns=["date", "feature", "weight", "gain", "cover", "total_gain", "total_cover"])
    ...
    if validation_percentage > 0:
        ...
        model = xgb.XGBClassifier(n_estimators=hp_iterations,
                                learning_rate=hp_learning_rate,
                                max_depth=hp_depth,
                                objective="multi:softmax",
                                random_state=hp_random_seed,
                                tree_method="hist",
                                device="cuda",
    ...
    total_cover_importance = model.get_booster().get_score(importance_type="total_cover")

    df_importances["feature"] = X_train.columns
    df_importances["date"] = "detached"
    df_importances["weight"] = df_importances["feature"].map(weight_importance)
    df_importances["gain"] = df_importances["feature"].map(gain_importance)
    df_importances["cover"] = df_importances["feature"].map(cover_importance)
    df_importances["total_gain"] = df_importances["feature"].map(total_gain_importance)
    df_importances["total_cover"] = df_importances["feature"].map(total_cover_importance)
...
df_results["pred_class"] = df_results["pred_class"] - 1



model_name = "xgboost_" + str(args.region) + "_" + run_time_and_date
if detached_test_dates:
...
mean_importances = df_importances.groupby("feature")[["weight", "gain", "cover", "total_gain", "total_cover"]].mean().sort_values(by="weight", ascending=False).reset_index()
