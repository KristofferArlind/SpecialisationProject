...
if (detached_test_dates):
    ...
    df_importances = pd.DataFrame(columns=["date", "feature", "bottom_importance", "middle_importance", "top_importance"])


    test_start_date = "2023-10-13"
    test_start_date = pd.to_datetime(test_start_date, format="%Y-%m-%d")
    test_interval = 7
    test_end_date = "2021-12-26"
    ...
    data_train, data_validation, data_test = utils.create_train_validation_test(data, test_date=None, validation=0, horizon_days=target_horizon)
    ...
    model = LogisticRegression(random_state=42, penalty='elasticnet', solver='saga', multi_class='multinomial', l1_ratio=hp_l1_ratio, max_iter=hp_max_iter, verbose=False)
    model.fit(X_train_scaled, y_train)

    df_importances = pd.DataFrame({"date": "detached", "feature": features, "bottom_importance": model.coef_[0], "middle_importance": model.coef_[1], "top_importance": model.coef_[2]})
...
# %%
    ...
    for test_date in test_dates:
        print("Test date: ")
        print(test_date)
        data_train, data_validation, data_test = utils.create_train_validation_test(data, test_date=test_date, validation=0, horizon_days=5)
        ...
        X_train = data_train.drop(columns=drop_cols)
        y_train = data_train["trr_5_fwd_class"]
        X_test = data_test.drop(columns=drop_cols)
        test_actual = data_test.copy()
        ...
        df_importance = pd.DataFrame({"date": test_date, "feature": features, "bottom_importance": model.coef_[0], "middle_importance": model.coef_[1], "top_importance": model.coef_[2]})
...
model_name = "logreg_" + str(args.region) + "_" + run_time_and_date
if detached_test_dates:
...
mean_bottom_importances = df_importances.groupby("feature")["bottom_importance"].mean().sort_values(ascending=False)
mean_middle_importances = df_importances.groupby("feature")["middle_importance"].mean().sort_values(ascending=False)
mean_top_importances = df_importances.groupby("feature")["top_importance"].mean().sort_values(ascending=False)

mean_bottom_importances.to_csv(f"results/{model_name}/mean_bottom_importances.csv")
mean_middle_importances.to_csv(f"results/{model_name}/mean_middle_importances.csv")
mean_top_importances.to_csv(f"results/{model_name}/mean_top_importances.csv")